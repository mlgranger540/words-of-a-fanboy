window.onload = async function(){
    const postData = await fetch(`/getPostsByFandom${window.location.pathname}`).then(function(response) {
        ...
        let currentFandomID = window.location.pathname.split("/").pop();
        ...
        for (i = 0; i < firstFandoms.length; i++) {
            ...
            if (firstFandoms[i].fandom_id === currentFandomID) {
                document.title = `Posts in ${firstFandoms[i].fandom_name} | In the Words of a Fanboy`;
                break;
            }
        }
